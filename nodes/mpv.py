...
class MPV(Node):
    ...
    def reconcile_links(self, links: List[Tuple[LinkPort, LinkPort]]):
        ...
        for channel in range(2):
            ...
            new_conn_names = [link[1].node.get_input_ports(link[1])[channel] for link in links]
    ...
    def link_to(self, src: LinkPort, tgt: LinkPort):
        (to_l, to_r) = tgt.node.get_input_ports(tgt)
        self._jack.connect(f'{self.id}:out_0', to_l)
        self._jack.connect(f'{self.id}:out_1', to_r)

    def unlink(self, src: LinkPort, tgt: LinkPort):
        (to_l, to_r) = tgt.node.get_input_ports(tgt)
        try:
            self._jack.disconnect(f'{self.id}:out_0', to_l)
        except jack.JackErrorCode as e:
            print("Unlink failed, ignoring:", e)
        try:
            self._jack.disconnect(f'{self.id}:out_1', to_r)
        except jack.JackErrorCode as e:
            print("Unlink failed, ignoring:", e)
